...
Looking at Figure 2, we can see that by filtering for states that passed at least one police reform policy (i.e. removing the 5 dots on the y-intercept), a clearer relationship between BLM tweeting proportion and police reform laws passed starts to emerge. However, we would like to restate that we are unsure if these observations are adding signal or noise to our data set. In Figure 3, removing the observations where no bills were passed has a less clear effect on the relationship between pro-BLM tweeting proportion and polcie reform laws passed.
